...
resource "aws_api_gateway_method" "sns_publish_post" {
  rest_api_id   = aws_api_gateway_rest_api.market_api.id
...
resource "aws_api_gateway_integration" "sns_publish_integration" {
  rest_api_id             = aws_api_gateway_rest_api.market_api.id
  resource_id             = aws_api_gateway_resource.sns_publish.id
  http_method             = aws_api_gateway_method.sns_publish_post.http_method
  integration_http_method = "POST"
  type                    = "AWS_PROXY"
  uri                     = aws_lambda_function.publish_lambda.invoke_arn
}

resource "aws_lambda_permission" "allow_apigw_invoke_publish" {
  statement_id  = "AllowAPIGatewayInvokePublish"
  action        = "lambda:InvokeFunction"
  function_name = aws_lambda_function.publish_lambda.function_name
